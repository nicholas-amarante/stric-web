...
export const CandidateCard: React.FC<CandidateCardProps> = ({ 
  ...
  return (
    <div className={`glass-card p-6 border transition-all duration-300 ${
      isInterviewSelected 
        ? 'border-emerald-500/50 bg-slate-900/90 shadow-glow-emerald' 
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-slate-800 to-slate-700 flex items-center justify-center font-bold text-base text-slate-200 border border-slate-700 shadow-sm">
      ...
      <div className="py-4 space-y-4">
        ...
        <div className="bg-slate-950/60 rounded-xl p-4 border border-slate-800/80 space-y-1.5">
        ...
        {candidatura.analiseIA?.dadosExtraidos && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-950/40 border border-slate-800/60 text-slate-300">
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-950/40 border border-slate-800/60 text-slate-300">
              <Briefcase size={15} className="text-brand-400 shrink-0" />
              <span>
        ...
        {candidatura.analiseIA?.requisitosAtendidos && candidatura.analiseIA.requisitosAtendidos.length > 0 && (
          <div>
            <div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-emerald-400 mb-1.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {candidatura.analiseIA.requisitosAtendidos.map((req, i) => (
                <span
                  key={i}
                  className="px-2.5 py-0.5 rounded-md text-xs font-medium bg-emerald-500/10 text-emerald-300 border border-emerald-500/20"
                >
                  {req}
                </span>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {candidatura.analiseIA.pontosAtencao.map((ponto, i) => (
                <span
                  key={i}
                  className="px-2.5 py-0.5 rounded-md text-xs font-medium bg-amber-500/10 text-amber-300 border border-amber-500/20"
                >
                  {ponto}
                </span>
              ))}
      ...
      <div className="pt-4 border-t border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs">
            <span className={`inline-flex items-center gap-1.5 font-mono ${
              isInterviewSelected ? 'text-white font-medium' : 'text-slate-400'
            }`}>
        ...
        <div className="flex items-center gap-2">
          ...
          {!isInterviewSelected ? (
            ...
            <span className="inline-flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-semibold bg-emerald-500/10 text-emerald-300 border border-emerald-500/20">
              <CheckCheck size={14} />
              Entrevista Agendada
            </span>
          )}

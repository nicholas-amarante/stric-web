...
export const JobCard: React.FC<JobCardProps> = ({ vaga, onApply }) => {
  return (
    <div className="glass-card p-6 flex flex-col justify-between border border-slate-800 hover:border-brand-500/50 hover:shadow-glow transition-all duration-300 group">
      <div>
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <span className="shrink-0 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-slate-800 text-slate-300 border border-slate-700/60">
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {vaga.requisitosObrigatorios.map((req, index) => (
              <span
                key={index}
                className="px-2.5 py-0.5 rounded-md text-xs font-medium bg-brand-500/10 text-brand-300 border border-brand-500/20"
              >
                {req}
              </span>
            ))}
      ...
      <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between mt-auto">
        <span className="text-xs text-slate-500">
          Status: <span className="text-emerald-400 font-medium">Inscrições Abertas</span>
        </span>
        <button

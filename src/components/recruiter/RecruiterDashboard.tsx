...
export const RecruiterDashboard: React.FC<RecruiterDashboardProps> = ({ 
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-3xl relative overflow-hidden border-slate-800">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8 pt-6 border-t border-slate-800/80">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-slate-800/80 text-brand-400 border border-slate-700">
              <Briefcase size={20} />
            </div>
            <div>
              <span className="text-xs text-slate-400 font-medium">Vagas Abertas</span>
              <p className="text-lg font-bold text-white">{vagas.length}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs text-slate-400 font-medium">Candidatos nesta Vaga</span>
              <p className="text-lg font-bold text-white">{candidaturas.length}</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-xl bg-slate-800/80 text-indigo-400 border border-slate-700">
              <TrendingUp size={20} />
            </div>
            <div>
              <span className="text-xs text-slate-400 font-medium">Aderência Média</span>
            ...
            return (
              <button
                ...
                className={`p-4 rounded-2xl border text-left transition-all duration-200 flex flex-col justify-between ${
                  isSelected
                    ? 'bg-slate-900 border-brand-500/60 shadow-glow'
                    : 'glass-card border-slate-800 hover:border-slate-700'
                }`}
              >
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-brand-500/10 text-brand-300 border border-brand-500/20">
                      Vaga Ativa
                    </span>
                ...
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-800/80 text-xs">
                  ...
                  <ChevronRight size={14} className={isSelected ? 'text-brand-400' : 'text-slate-600'} />
                </div>
              </button>
            );
          })}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {selectedVaga.requisitosObrigatorios.map((req, idx) => (
                  <span key={idx} className="text-xs px-2.5 py-0.5 rounded-md bg-brand-500/10 text-brand-300 border border-brand-500/20">
                    {req}
                  </span>
                ))}
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {selectedVaga.requisitosDesejaveis.map((req, idx) => (
                    <span key={idx} className="text-xs px-2.5 py-0.5 rounded-md bg-emerald-500/10 text-emerald-300 border border-emerald-500/20">
                      {req}
                    </span>
                  ))}
                </div>
      ...
      {isLoadingCandidaturas ? (
        <div className="glass-panel p-12 rounded-2xl flex flex-col items-center justify-center space-y-3">
          ...
          <p className="text-xs text-slate-400">Carregando candidatos e análises da IA...</p>
        </div>
      ) : (
        <CandidateRanking
